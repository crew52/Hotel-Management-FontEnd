...
const authService = {
  ...
  login: async (usernameOrEmail, password) => {
    ...
    try {
      ...
      if (success && data) {
        // Lấy token và thông tin người dùng từ data
        const { accessToken, tokenType, userDetails } = data;
        
        // Lưu token nếu tồn tại
        if (accessToken) {
          const token = tokenType ? `${tokenType} ${accessToken}` : accessToken;
          localStorage.setItem('token', token);
          console.log('Token stored in localStorage');
        }
        
        // Lưu thông tin người dùng nếu tồn tại
        if (userDetails) {
          localStorage.setItem('user', JSON.stringify(userDetails));
          console.log('User info stored in localStorage:', userDetails);
        }
        
        // Trả về dữ liệu cho AuthContext
        return {
          token: accessToken,
          user: userDetails
        };
